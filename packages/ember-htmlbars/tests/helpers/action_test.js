...
import {
  ActionHelper,
  actionHelper
} from "ember-htmlbars/helpers/action";
...
QUnit.module("HTMLBars {{action}} helper", {
  setup: function() {
...
test("Inside a yield, the target points at the original target", function() {
  ...
  view = EmberView.create({
    ...
    template: compile('{{#with obj}}{{#if truthy}}{{#view component}}{{#if truthy}}<p {{action "wat"}} class="wat">{{boundText}}</p>{{/if}}{{/view}}{{/if}}{{/with}}')
  });
...
test("should register an event handler", function() {
  ...
  ok(HandlebarsActionHelper.registeredActions[actionId], "The action was registered");
...
test("should work properly in an #each block", function() {
  ...
  view = EmberView.create({
    ...
    template: compile('{{#each view.items}}<a href="#" {{action "edit"}}>click me</a>{{/each}}')
  });
...
test("should unregister event handlers on inside virtual views", function() {
  ...
  view = EmberView.create({
    template: compile('{{#each view.things}}<a href="#" {{action "edit"}}>click me</a>{{/each}}'),
    things: things
  });
  ...
  ok(!HandlebarsActionHelper.registeredActions[actionId], "After the virtual view was destroyed, the action was unregistered");
});
...
test("should send the view, event and current Handlebars context to the action", function() {
  ...
  view = EmberView.create({
    aContext: aContext,
    template: compile('{{#with view.aContext}}<a id="edit" href="#" {{action "edit" this target="aTarget"}}>edit</a>{{/with}}')
  });
...
if (!Ember.FEATURES.isEnabled('ember-routing-drop-deprecated-action-style')) {
  test("should invoke a handler defined directly on the target (DEPRECATED)", function() {
    ...
    view = EmberView.create({
      controller: controller,
      template: compile('<button {{action "edit"}}>edit</button>')
    });
