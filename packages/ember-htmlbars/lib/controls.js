...
* `form`
  ...
  @param {Hash} options
*/
export function inputHelper(params, options, env) {
  Ember.assert('You can only pass attributes to the `input` helper, not arguments', params.length === 0);

  var hash = options.hash || {}, // HTMLBARS-TODO: Normalize?
  ...
  if (inputType === 'checkbox') {
    ...
    return env.helpers.view([Checkbox], options, env);
  } else {
    if (inputType) { hash.type = inputType; }
    hash.onEvent = onEvent || 'enter';
    return env.helpers.view([TextField], options, env);
  }
}
