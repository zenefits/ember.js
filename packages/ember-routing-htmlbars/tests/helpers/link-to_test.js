...
import { defaultEnv } from "ember-routing-htmlbars";
...
test("The {{link-to}} helper does not disregard currentWhen when it is given explicitly for a resource", function() {
  ...
  Ember.TEMPLATES.index = compile("<h3>Home</h3>{{outlet}}");
  Ember.TEMPLATES['index/about'] = compile("{{#link-to 'items' id='other-link' currentWhen='index'}}ITEM{{/link-to}}");
...
test("The {{link-to}} helper moves into the named route with context", function() {
  ...
  Ember.TEMPLATES.about = compile("<h3>List</h3><ul>{{#each controller}}<li>{{#link-to 'item' this}}{{name}}{{/link-to}}</li>{{/each}}</ul>{{#link-to 'index' id='home-link'}}Home{{/link-to}}");
...
test("The {{link-to}} helper binds some anchor html tag common attributes", function() {
  Ember.TEMPLATES.index = compile("<h3>Home</h3>{{#link-to 'index' id='self-link' title='title-attr' rel='rel-attr'}}Self{{/link-to}}");
  bootApplication();
...
test("The non-block form {{link-to}} helper moves into the named route with context", function() {
  ...
  Ember.TEMPLATES.index = compile("<h3>Home</h3><ul>{{#each controller}}<li>{{link-to name 'item' this}}</li>{{/each}}</ul>");
  Ember.TEMPLATES.item = compile("<h3>Item</h3><p>{{name}}</p>{{#link-to 'index' id='home-link'}}Home{{/link-to}}");
